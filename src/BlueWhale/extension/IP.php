...
class IP
{
    ...
    public static function find($ip) {
        ...
        if (isset(self::$cached[$nip]) === TRUE) {
            return is_array(self::$cached[$nip]) ? self::$cached[$nip] : [self::$cached[$nip]];
        }
    ...
    public static function getFrom(string $ip) {
        return implode("", self::find($ip));
    }

    public static function isDatabaseExists() {
        $fp = fopen('plugins/WTask/database/ipdata.wpk', 'rb');
        if ($fp === false) {
            return false;
        } else {
            fclose($fp);
    ...
    private static function init() {
        if (self::$fp === NULL) {
            ...
            self::$fp = fopen('plugins/WTask/database/ipdata.wpk', 'rb');
            if (self::$fp === FALSE) {
                throw new \Exception('Invalid ipdata.wpk file!');
            }

...
class PlayerMoveListener implements Listener, TaskListener
{
    public $plugin;
    public $task;
    public $api;
    ...
    public function onBreak(PlayerMoveEvent $event) {
        $tn = $this->tn;
        $t = new ActTaskAPI($event, $event->getPlayer(), $this->api);
        $t->writePrivateData("to|" . $event->getTo()->x . ":" . $event->getTo()->y . ":" . $event->getTo()->z . ":" . $event->getTo()->level->getFolderName());
        $ID = 0;

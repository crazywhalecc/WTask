...
class PlayerDeathListener implements Listener, TaskListener
{
    public $plugin;
    public $task;
    public $api;
    ...
    public function onBreak(PlayerDeathEvent $event) {
        $tn = $this->tn;
        ...
        while (isset($this->task[$ID])) {
            ...
            switch ($inside["type"]) {
                case "设置保留背包":
                    $t->setKeepInv($inside["function"]);
                    break;
                case "设置保留经验":
                    $t->setKeepExp($inside["function"]);
                    break;
                case "c":
